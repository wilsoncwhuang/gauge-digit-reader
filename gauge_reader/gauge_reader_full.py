...
def calibrate_gauge(gauge_number, file_type):
    ...
    img = cv2.imread('data/gauge (%s).%s' %(gauge_number, file_type))
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20, np.array([]), 50, 50, int(width*0.35), int(width*0.48))
    print(circles)
    ...
    cv2.imwrite('gauge_detection/gauge-%s-circle.%s' % (gauge_number, file_type), img)
    ...
    cv2.imwrite('gauge_detection/gauge-%s-calibration.%s' % (gauge_number, file_type), img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    for thresh in thresh_arr:
        ...
        #cv2.imwrite('gauge_detection/gauge-%s-dst2-%d.%s' % (gauge_number, thresh, file_type), dst2)
        minLineLength = 0.4 * r
        maxLineGap = 0
        ...
        if lines is not None:
            ...
            if len(final_line_list) > 0:
                ...
                cv2.imwrite('gauge_detection/gauge-%s-lines-%d.%s' % (gauge_number, thresh, file_type), img)

                value = process_angle / (max_angle - min_angle) * (max_value - min_value) + min_value
                return value
         
def main(args):
    x, y, r = calibrate_gauge(args.gauge_number, args.file_type)
    img = cv2.imread('data/gauge (%s).%s' % (args.gauge_number, args.file_type))
    val = get_current_value(img, args.min_angle, args.max_angle, args.min_value, args.max_value, x, y, r, args.gauge_number, args.file_type)
    return val

if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--gauge_number', default = 11,      type = int)
    parser.add_argument('--file_type',    default = 'jpg',  type = str)
    parser.add_argument('--min_angle',    default = 45,     type = float)
    parser.add_argument('--max_angle',    default = 320,    type = float)
    parser.add_argument('--min_value',    default = 0,      type = float)
    parser.add_argument('--max_value',    default = 150,    type = float)
    parser.add_argument('--unit',         default = 'PSI',  type = str)
